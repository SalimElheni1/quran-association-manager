...
async function unlinkWithRetry(filePath, retries = 5, delay = 100) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.info(`Successfully unlinked ${filePath}`);
      return;
    } catch (error) {
...
async function replaceDatabase(importedDbPath, password) {
  ...
  try {
    ...
    console.info('Salt configuration updated from backup.');
    if (fsSync.existsSync(currentDbPath)) {
      console.info(`Deleting old database file at ${currentDbPath}...`);
      await unlinkWithRetry(currentDbPath);
    }
    console.info('Initializing new database with imported salt...');
    await initializeDatabase(password);
    console.info('New database initialized successfully.');
    console.info('Executing SQL script to import data...');
    ...
    console.info('Database import successful. The app will now restart.');
    app.relaunch();
    app.quit();
    return {
      success: true,
