...
const runBackup = async (settings, backupFilePath) => {
  console.info('SQL-based backup process started...');
  ...
  try {
    ...
    console.info(message, `Path: ${backupFilePath}`);
...
const startScheduler = (settings) => {
  ...
  if (!settings.backup_enabled) {
    console.info('Backup scheduler is disabled.');
    return;
  }

  console.info(`Backup scheduler started. Frequency: ${settings.backup_frequency}.`);
  ...
  schedulerIntervalId = setInterval(
    async () => {
      ...
      if (settings.backup_enabled && isBackupDue(settings)) {
        console.info('Scheduled backup is due. Running now...');
