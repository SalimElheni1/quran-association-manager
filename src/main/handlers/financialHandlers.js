...
async function handleGetTransactions(event, filters) {
  try {
    ...
    if (searchTerm) {
      sql += ' AND (t.description LIKE ? OR t.related_person_name LIKE ? OR t.voucher_number LIKE ?)';
...
async function handleDeleteTransaction(event, transactionId) {
  try {
    ...
    const transaction = await db.getQuery('SELECT * FROM transactions WHERE id = ?', [transactionId]);
    if (!transaction) {
      throw new Error('العملية المالية غير موجودة');
    }
...
async function handleGetFinancialSummary(event, period) {
  try {
    const tableCheck = await db.getQuery(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='transactions'"
    );
    ...
    const transactionCount = income.reduce((sum, r) => sum + r.count, 0) + expenses.reduce((sum, r) => sum + r.count, 0);
...
async function handleGetInKindCategories(event) {
  try {
    ...
    if (!tableCheck) return [];
    return await db.allQuery('SELECT * FROM in_kind_categories WHERE is_active = 1 ORDER BY is_system, name');
  } catch (error) {
...
async function handleExportFinancialReportExcel(event, data) {
  try {
    ...
    worksheet.addRow(['الرصيد الصافي', ((summary.totalIncome || 0) - (summary.totalExpenses || 0)).toFixed(2)]);
    worksheet.addRow([]);
...
function registerFinancialHandlers() {
  ...
  ipcMain.handle('financial:export-pdf', requireRoles(['Superadmin', 'Administrator', 'FinanceManager'])(handleExportFinancialReportPDF));
  ipcMain.handle('financial:export-excel', requireRoles(['Superadmin', 'Administrator', 'FinanceManager'])(handleExportFinancialReportExcel));
