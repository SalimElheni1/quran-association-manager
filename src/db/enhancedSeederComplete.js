...
const getDummyClasses = (teacherIds) => {
  return [
    { name: 'فصل التجويد للرجال', gender: 'men', status: 'active', teacher_id: teacherIds.male[0], capacity: 20 },
    { name: 'فصل الحفظ للنساء', gender: 'women', status: 'active', teacher_id: teacherIds.female[0], capacity: 15 },
    { name: 'فصل الأطفال التمهيدي', gender: 'kids', status: 'pending', teacher_id: teacherIds.male[1], capacity: 12 },
    { name: 'فصل اللغة العربية', gender: 'all', status: 'active', teacher_id: teacherIds.male[0], capacity: 25 },
    { name: 'فصل الفقه اليومي', gender: 'men', status: 'active', teacher_id: teacherIds.male[2], capacity: 18 },
    { name: 'فصل الحفظ المكثف', gender: 'women', status: 'active', teacher_id: teacherIds.female[1], capacity: 16 },
    { name: 'فصل الأطفال المتقدم', gender: 'kids', status: 'active', teacher_id: teacherIds.female[0], capacity: 10 },
    { name: 'فصل التفسير العام', gender: 'all', status: 'active', teacher_id: teacherIds.male[1], capacity: 30 },
  ];
};
...
async function seedUsers() {
  ...
  const { count } = await getQuery("SELECT COUNT(*) as count FROM users WHERE role != 'Superadmin'");
  if (count > 0) return;
...
async function seedAttendance() {
  ...
  const { count } = await getQuery('SELECT COUNT
