...
describe('financialHandlers', () => {
    ...
    describe('handleAddExpense', () => {
        it('should add new expense and return the new record', async () => {
            const expenseData = { category: 'Office Supplies', amount: 75.50 };
    ...
    describe('handleAddDonation', () => {
        it('should add cash donation successfully', async () => {
            ...
            expect(db.runQuery).toHaveBeenCalledWith(
                expect.any(String),
                ['Ahmed Ali', 1000, undefined, undefined, 'Cash', undefined, undefined, undefined]
            );
            expect(result).toEqual(newRecord);
        });
        ...
        it('should add in-kind donation successfully', async () => {
            ...
            expect(db.runQuery).toHaveBeenCalledWith(
                expect.any(String),
                ['Fatima Hassan', undefined, undefined, undefined, 'In-kind', '50 Quran copies', undefined, undefined]
            );
            expect(result).toEqual(newRecord);
        });
    });
