...
jest.mock('react-bootstrap', () => ({
    ...
    Button: ({ children, onClick, variant, size }) => <button onClick={onClick} className={`btn-${variant} btn-${size}`}>{children}</button>,
...
jest.mock('@renderer/components/UserFormModal', () => ({ show, handleClose, onSaveSuccess, user }) => {
    if (!show) return null;
    return (
...
describe('UsersPage', () => {
    const mockUsers = [
        { id: 1, matricule: 'U001', first_name: 'John', last_name: 'Doe', username: 'johndoe', role: 'Admin', status: 'active' },
        { id: 2, matricule: 'U002', first_name: 'Jane', last_name: 'Doe', username: 'janedoe', role: 'Manager', status: 'inactive' },
    ];
