...
jest.mock('../src/main/services/voucherService', () => ({
...
describe('importManager - Extended Tests', () => {
  ...
  describe('importExcelData - Generic Logic', () => {
    ...
    it('should return an error if the Excel file cannot be read', async () => {
        ...
        await expect(importExcelData('nonexistent.xlsx', ['الطلاب'])).rejects.toThrow('File not found');
    });
  ...
  describe('processStudentRow', () => {
    ...
    it('should update an existing student successfully', async () => {
        const rowData = { 'الرقم التعريفي': 'S-EXISTING', 'الاسم واللقب': 'Updated Name', 'الحالة': 'غير نشط' };
        const mockRow = createMockRow(rowData);
  ...
  describe('processUserRow', () => {
    ...
    it('should create a new user successfully', async () => {
        const rowData = { 'اسم المستخدم': 'new.user', 'الاسم الأول': 'New', 'اللقب': 'User', 'الدور': 'Administrator', 'نوع التوظيف': 'Full-time' };
        const mockRow = createMockRow(rowData);
        ...
        expect(runQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO users'), expect.any(Array));
        expect(runQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO user_roles'), [100, 1]);
    ...
    it('should rollback transaction if role assignment fails', async () => {
        const rowData = { 'اسم المستخدم': 'fail.user', 'الاسم الأول': 'Fail', 'اللقب': 'User', 'الدور': 'NonExistentRole', 'نوع التوظيف': 'Part-time' };
        const mockRow = createMockRow(rowData);
        ...
        await expect(processUserRow(mockRow, mockHeaderRow)).rejects.toThrow('Role "NonExistentRole" not found.');
  ...
  describe('processAttendanceRow', () => {
    ...
    it('should create an attendance record successfully', async () => {
        const rowData = { 'الرقم التعريفي للطالب': 'S-VALID', 'اسم الفصل': 'Class A', 'التاريخ': '2024-01-01', 'الحالة': 'حاضر' };
        const mockRow = createMockRow(rowData);
    ...
    it('should return an error if the student is not found', async () => {
        const rowData = { 'الرقم التعريفي للطالب': 'S-INVALID', 'اسم الفصل': 'Class A', 'التاريخ': '2024-01-01', 'الحالة': 'حاضر' };
        const mockRow = createMockRow(rowData);
    ...
    it('should return an error if the class is not found', async () => {
        const rowData = { 'الرقم التعريفي للطالب': 'S-VALID', 'اسم الفصل': 'Invalid Class', 'التاريخ': '2024-01-01', 'الحالة': 'حاضر' };
        const mockRow = createMockRow(rowData);
  ...
  describe('processTransactionRow', () => {
    ...
    it('should create an income transaction successfully', async () => {
        const rowData = { 'النوع': 'مدخول', 'الفئة': 'التبرعات النقدية', 'المبلغ': 100, 'التاريخ': '2024-01-01', 'طريقة الدفع': 'نقدي' };
        const mockRow = createMockRow(rowData);
        ...
        expect(runQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO transactions'), expect.arrayContaining(['I-2024-001', 'INCOME', 100]));
    ...
    it('should fail if amount is > 500 and payment method is cash', async () => {
        const rowData = { 'النوع': 'مدخول', 'الفئة': 'التبرعات النقدية', 'المبلغ': 600, 'التاريخ': '2024-01-01', 'طريقة الدفع': 'نقدي' };
        const mockRow = createMockRow(rowData);
    ...
    it('should fail if category is invalid for the given type', async () => {
        const rowData = { 'النوع': 'مصروف', 'الفئة': 'التبرعات النقدية', 'المبلغ': 50, 'التاريخ': '2024-01-01', 'طريقة الدفع': 'نقدي' };
        const mockRow = createMockRow(rowData);
  ...
  describe('processInventoryRow', () => {
    ...
    it('should fail if inventory item already exists', async () => {
        const rowData = { 'اسم العنصر': 'Existing Item', 'الفئة': 'Supplies', 'الكمية': 10, 'قيمة الوحدة': 5 };
        const mockRow = createMockRow(rowData);
